...
function fillBoard() {
	...
	for(let i = 0; i < board.length; i++){
		...
		`<div class="card" data-id="${board[i].id}">
			...
			<div class="back"><img src="${board[i].img}" 
			alt="${board[i].name}"></div>
		</div>`;
...
function shuffle(array){
	...
	while (counter > 0) { //Пока в массиве есть элементы
		index = Math.floor(Math.random() * counter);//Выбираем случайный индекс
		counter--;//Уменьшаем счетчик на 1
		temp = array[counter];//поменять местами последний элемент с ним
